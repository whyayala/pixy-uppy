...
pub fn platform_dir() -> &'static str {
    if cfg!(target_os = "windows") { "win64" } else { "linux64" }
}
...
pub fn resolve_tool(tool_name: &str) -> Result<PathBuf, PixyError> {
    if let Ok(p) = which(tool_name) { return Ok(p); }
    ...
    let candidate = cwd.join("third_party").join("bin").join(platform_dir()).join(tool_name);
    if candidate.exists() { return Ok(candidate); }

    // relative to executable dir
    if let Ok(exe) = env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("third_party").join("bin").join(platform_dir()).join(tool_name);
            if candidate.exists() { return Ok(candidate); }
        }
    }

    Err(PixyError::CommandNotFound(Box::leak(tool_name.to_string().into_boxed_str())))
}
